...
class StreamingVocGan:
    ...
    def __init__(self, model_path, is_streaming: bool = False):
        ...
        self.model = ModifiedGenerator(
            mel_channel=self.hp.audio.n_mel_channels,
            n_residual_layers=self.hp.model.n_residual_layers,
            is_streaming=is_streaming,
            ratios=self.hp.model.generator_ratio,
            mult=self.hp.model.mult,
    ...
    def waveform_to_mel_spectrogram(
        waveform: Union[torch.Tensor, np.array],
        original_sampling_rate: int,
        mel_channel_count: int = 80,
        ...
        # Input management
        if not type(waveform) == type(torch.Tensor):
        ...
        for i in range(waveform.shape[0]):
            waveform[i] = waveform[i] / torch.max(torch.abs(waveform[i]))

        # Converting step parameters according to current sampling rate
        hop_length = (int)(
            StreamingVocGan.WAVE_FRAMES_PER_HOP
            / StreamingVocGan.WAVE_SAMPLING_RATE
            * original_sampling_rate
        )
        win_length = (int)(
            StreamingVocGan.WAVE_FRAMES_PER_WINDOW
            / StreamingVocGan.WAVE_SAMPLING_RATE
            * original_sampling_rate
        )
        ...
        stft = TacotronSTFT(
            ...
            n_mel_channels=mel_channel_count,
            sampling_rate=original_sampling_rate,
            mel_fmin=min_frequency,
            mel_fmax=max_frequency,
        )
    ...
    def plot(
        ...
        waveform_standard: torch.Tensor,
        waveform_streaming_slices: List[torch.Tensor] = None,
        slice_processing_times: List[float] = None,
    ) -> None:
        ...
        if type(slice_processing_times) != type(None):
            if type(waveform_streaming_slices) == type(None):
                raise ValueError(
                    "If the argument slice_processing_times is provided, then waveform_streaming_slices has to be provided."
                )

        # Cast to numpy arrays
        waveform_standard = waveform_standard.detach().numpy()
        if waveform_streaming_slices != None:
            waveform_streaming_slices = [
                slice.detach().numpy() for slice in waveform_streaming_slices
            ]

        # Count plots
        plot_count = (
            2
            + (1 if type(waveform_streaming_slices) != type(None) else 0)
            + (1 if type(slice_processing_times) != type(None) else 0)
        )

        # Time ticks
        time_frame_count = mel_spectrogram.shape[-1]
        total_seconds = (time_frame_count - 1) * StreamingVocGan.TIMING_CONVENTIONS[
            "Seconds Per Spectrogram Hop"
        ] + StreamingVocGan.TIMING_CONVENTIONS["Seconds Per Spectrogram Window"]
        ...
        if type(waveform_streaming_slices) != type(None):
            ...
            if type(slice_processing_times) != type(None):
                ...
                slice_starts = np.cumsum(
                    [0] + [slice.shape[-1] for slice in waveform_streaming_slices[:-1]]
                )  # Now the first number is the first slice's starting index (0), last number is last slice's starting index
                slice_stops = np.cumsum(
                    [slice.shape[-1] for slice in waveform_streaming_slices]
                )  # Now the first number is the stop index (exclusive) of the first slice, last number is stop index of last slice
                slice_durations = np.array(
                    [
                        slice.shape[-1] / StreamingVocGan.WAVE_SAMPLING_RATE
                        for slice in waveform_streaming_slices
                    ]
                )
                efficiencies = [
                    (
                        slice_processing_times[i] / slice_durations[i]
                        if slice_durations[i] > 0
                        else 0
                    )
                    for i in range(len(slice_durations))
                ]
                plt.bar(
                    x=slice_starts + (slice_stops - slice_starts) / 2,
                    height=efficiencies,
                    width=slice_stops - slice_starts,
                    fill=False,
                )
                ...
                textstr = ",   ".join(
                    (
                        ...
                        r"5 percent trimmed average Audio Duration = %.4f sec"
                        % (stats.trim_mean(slice_durations, 0.05)),
                        r"5 percent trimmed average Processing Time = %.4f sec"
                        % (stats.trim_mean(slice_processing_times, 0.05)),
                    )
                )
        ...
        tick_locations = np.arange(
            stop=time_frame_count, step=StreamingVocGan.WAVE_SAMPLING_RATE
        )
    ...
    def mel_spectrogram_to_waveform(
        self, mel_spectrogram: torch.Tensor, is_final_slice: bool = False
        ...
        with torch.no_grad():
            ...
            if len(mel_spectrogram.shape) == 2:
                mel_spectrogram = mel_spectrogram.unsqueeze(
                    0
                )  # Now shape == [instance count, channel count, time frame count]

            # Predict
            if is_final_slice:
                streaming.StreamingModule.final_slice()
    ...
    @staticmethod
    def slice_duration_to_frame_count(
        spectrogram_time_frame_count, target_seconds_per_slice: float
    ...
    @staticmethod
    def slice_generator(
        mel_spectrogram: torch.Tensor, time_frames_per_slice: float
        ...
        for i in range(slice_count):
            x_i = mel_spectrogram[
                :, i * time_frames_per_slice : (i + 1) * time_frames_per_slice
            ]
    ...
    def save(waveform: torch.Tensor, file_path: str) -> None:
        ...
        scipy.io.wavfile.write(
            filename=file_path, rate=StreamingVocGan.WAVE_SAMPLING_RATE, data=waveform
        )
...
if __name__ == "__main__":
    ...
    file_name = "sub-06_orig_audio.wav"
    input_path = path.join("..", "..", "Dataset_Word", file_name)
    ...
    if samplerate == 48000:
        waveform = scipy.signal.decimate(waveform, 3)
        samplerate /= 3
    ...
    mel_spectrogram = StreamingVocGan.waveform_to_mel_spectrogram(
        waveform=waveform, original_sampling_rate=samplerate
    )
    ...
    waveform_standard, standard_processing_time = (
        standard_model.mel_spectrogram_to_waveform(mel_spectrogram=mel_spectrogram)
    )
    print(
        f"Converting the spectrogram to waveform took {standard_processing_time} seconds in standard mode."
    )
    ...
    # time_frames_per_slice, actual_seconds_per_slice, slice_count = (
    #     StreamingVocGan.slice_duration_to_frame_count(
    #         spectrogram_time_frame_count=mel_spectrogram.shape[-1],
    #         target_seconds_per_slice=0.035,
    #     )
    # )
    # print(f"Each spectrogram slice contains {time_frames_per_slice} frames.")
    # print(
    #     f"Each spectrogram slice provides {actual_seconds_per_slice} seconds of new audio."
    # )  # Note, the output slices will have duration of frames_per_slice * StreamingVocGan.TIMING_CONVENTIONS['Seconds Per Spectrogram Hop'] which is a bit shorter. The surplus is saved in the state of the vocgan during streaming
    # generator = streaming_model.slice_generator(
    #     mel_spectrogram=mel_spectrogram, time_frames_per_slice=time_frames_per_slice
    # )
    ...
    #     waveform_streaming_slices[i], slice_processing_times[i] = (
    #         streaming_model.mel_spectrogram_to_waveform(
    #             mel_spectrogram=x_i, is_final_slice=is_final_slice
    #         )
    #     )
    # print(
    #     f"Converting the spectrogram to waveform took {np.sum(slice_processing_times)} seconds in streaming mode."
    # )
    ...
    # # Plot
    # StreamingVocGan.plot(
    #     mel_spectrogram=mel_spectrogram,
    #     waveform_standard=waveform_standard,
    #     waveform_streaming_slices=waveform_streaming_slices,
    #     slice_processing_times=slice_processing_times,
    # )
