...
class TransformerLayer(nn.Module):
    def __init__(self, d_model, n_head, d_ff, dropout=0.5):
...
class TransformerEncoder(nn.Module):
    def __init__(self, num_layers, d_model, n_head, d_ff, dropout=0.5):
        super(TransformerEncoder, self).__init__()
        self.layers = nn.ModuleList([TransformerLayer(d_model, n_head, d_ff, dropout) for _ in range(num_layers)])
