...
def hilbert3(x): return scipy.signal.hilbert(
...
def extractHG(data, sr, windowLength=0.05, frameshift=0.01):
    ...
    sos = scipy.signal.iirfilter(
        4, [70/(sr/2), 170/(sr/2)], btype='bandpass', output='sos')
    data = scipy.signal.sosfiltfilt(sos, data, axis=0)
    # Attenuate first harmonic of line noise
    sos = scipy.signal.iirfilter(
        4, [98/(sr/2), 102/(sr/2)], btype='bandstop', output='sos')
    data = scipy.signal.sosfiltfilt(sos, data, axis=0)
    # Attenuate second harmonic of line noise
    sos = scipy.signal.iirfilter(
        4, [148/(sr/2), 152/(sr/2)], btype='bandstop', output='sos')
    data = scipy.signal.sosfiltfilt(sos, data, axis=0)
...
def stackFeatures(features, modelOrder=4, stepSize=5):
    ...
    featStacked = np.zeros(
        (features.shape[0]-(2*modelOrder*stepSize), (2*modelOrder+1)*features.shape[1]))
    for fNum, i in enumerate(range(modelOrder*stepSize, features.shape[0]-modelOrder*stepSize)):
        ef = features[i-modelOrder*stepSize:i +
                      modelOrder*stepSize+1:stepSize, :]
...
def downsampleLabels(labels, sr, windowLength=0.05, frameshift=0.01):
    ...
    """
    numWindows = int(
        np.floor((labels.shape[0]-windowLength*sr)/(frameshift*sr)))
    ...
    for w in range(numWindows):
        ...
        newLabels[w] = hist[0][np.argsort(
            hist[1])][-1].encode("ascii", errors="ignore").decode()
    return newLabels
...
def extractMelSpecs(audio, sr, windowLength=0.05, frameshift=0.01):
    ...
    """
    numWindows = int(
        np.floor((audio.shape[0]-windowLength*sr)/(frameshift*sr)))
...
def nameVector(elecs, modelOrder=4):
    ...
    """
    names = matlib.repmat(elecs.astype(
        np.dtype(('U', 10))), 1, 2 * modelOrder + 1).T
...
if __name__ == "__main__":
    ...
    participants = pd.read_csv(os.path.join(
        path_bids, 'participants.tsv'), delimiter='\t')
    for p_id, participant in enumerate(participants['participant_id'][5:6]):

        # Load data
        io = NWBHDF5IO(os.path.join(path_bids, participant, 'ieeg',
                       f'{participant}_task-wordProduction_ieeg.nwb'), 'r')
        nwbfile = io.read()
        # sEEG
        ...
        io.close()
        # channels
        channels = pd.read_csv(os.path.join(path_bids, participant, 'ieeg',
                               f'{participant}_task-wordProduction_channels.tsv'), delimiter='\t')
        ...
        melSpec = extractMelSpecs(
            scaled, audio_sr, windowLength=winL, frameshift=frameshift)

        # Align to EEG features
        words = downsampleLabels(
            words, eeg_sr, windowLength=winL, frameshift=frameshift)
        words = words[modelOrder*stepSize:words.shape[0]-modelOrder*stepSize+1]
        melSpec = melSpec[modelOrder *
                          stepSize:melSpec.shape[0]-modelOrder*stepSize, :]
        ...
        np.save(os.path.join(path_output,
                f'{participant}_procWords.npy'), words)
