...
from . import reconstructWave as rW
from . import MelFilterBank as mel
...
if __name__ == "__main__":
    ...
    pts = ["sub-%02d" % i for i in range(6, 7)]
    ...
    for pNr, pt in tqdm(enumerate(pts)):
        ...
        for k, (train, test) in enumerate(kf.split(data)):
            ...
            for specBin in range(spectrogram.shape[1]):
                if np.any(np.isnan(rec_spec)):
                    tqdm.write(
                        "%s has %d broken samples in reconstruction"
                        % (pt, np.sum(np.isnan(rec_spec)))
                    )
        ...
        for randRound in range(numRands):
            ...
            splitPoint = np.random.choice(
                np.arange(
                    int(spectrogram.shape[0] * 0.1), int(spectrogram.shape[0] * 0.9)
                )
            )
            # Swap the dataset on the splitting point
            shuffled = np.concatenate(
                (spectrogram[splitPoint:, :], spectrogram[:splitPoint, :])
            )
        ...
        reconstructedWav = createAudio(
            rec_spec, audiosr=audiosr, winLength=winLength, frameshift=frameshift
        )
        wavfile.write(
            os.path.join(result_path, f"{pt}_predicted.wav"),
            int(audiosr),
            reconstructedWav,
        )
