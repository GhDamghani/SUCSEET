...
def stackFeatures(features, modelOrder=4, stepSize=5):
    ...
    featStacked = np.zeros(
        (
            features.shape[0] - (2 * modelOrder * stepSize),
            (2 * modelOrder + 1) * features.shape[1],
        )
    )
    for fNum, i in enumerate(
        range(modelOrder * stepSize, features.shape[0] - modelOrder * stepSize)
    ):
        ef = features[
...
def downsampleLabels(labels, sr, windowLength=0.05, frameshift=0.01):
    ...
    for w in range(numWindows):
        ...
        newLabels[w] = (
            hist[0][np.argsort(hist[1])][-1].encode("ascii", errors="ignore").decode()
        )
    return newLabels
...
if __name__ == "__main__":
    ...
    for p_id, participant in enumerate(participants["participant_id"]):
        ...
        np.save(
            os.path.join(path_output, f"{participant}_feat_names.npy"), feature_names
        )
